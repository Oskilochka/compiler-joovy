...
def translateProgram():
    ...
    # Skip all declarations (they were processed during parsing)
    ...
    while numRow <= len_tableOfSymb:
        ...
        # Skip variable/constant declarations
...
def skipDeclaration():
    """Skips variable or constant declaration"""
    numLine, lex, tok = getSymb()
...
def skipExpression():
    """Skips expression (for declarations)"""
    # Simple version - skip until comma, semicolon, or closing bracket
    while True:
        numLine, lex, tok = getSymb()
        if lex in (',', ';', ')') or tok == 'PAR_OP' and lex == '}':
            break
        nextSymb()
...
def translateIf():
    ...
    Postfix scheme:
    Expression m1 JF Block1 m2 JMP m1: Block2 m2:

    Where:
    - If Expression is false, jump to m1 (else block)
    - After Block1, unconditional jump to m2 (end)
    - m1: start of else block
    - m2: end of if statement
    """
